...
export default function CategoryToAdd() {
  ...
  const handleOnSubmit = (e) => {
    e.preventDefault();
    dispatch(createcategoryAction({ ...formData, file }));
  };
  const { loading, error, isAdded } = useSelector((state) => state.categories);
  const fileHandleChange = (e) => {
    const newFiles = e.target.files[0];
    let newErr = [];
    if (newFiles?.size > 1000000) newErr.push(`${newFiles.name} to large`);

    if (!newFiles?.type?.startsWith("image/"))
      newErr.push(`${newFiles.name} not an image`);
  ...
  return (
    <>
      ...
      {fileErr.length > 0 && <ErrorComponent message={fileErr[0]} />}
